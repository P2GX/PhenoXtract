...
transformation is applied. Generally, this should be set to "String". But there are situations where "Boolean", "Float64" 
...
PhenoXtract expects the data in a table to have a certain format before it can be collected and put into Phenopackets. For example,
...
strategies can be found here: [strategy list]
...
Phenopackets [link to MetaData docs]. The `cohort_name` will become the prefix of the Phenopacket IDs. For example, 
