...
fn build_hgnc_test_client(temp_dir: &Path) -> CachedHGNCClient {
    CachedHGNCClient::new(temp_dir.join(
        "test_hgnc_cache"), HGNCClient::default()).unwrap()
    }
    fn build_hgvs_test_client(temp_dir: &Path) -> CachedHGVSClient {
        CachedHGVSClient::new(temp_dir.join(
            "test_hgvs_cache"), HGVSClient::default()).unwrap()
        }
...
fn test_complete_keio(temp_dir: TempDir) {
    ...
    let expected_ids: HashSet<&str> = HashSet::from([
        "patient_0001", "patient_0002", "patient_0003", "patient_0004", "patient_0005"
    ]);
    ...
    for p in json_files {
        ...
        assert!(
            expected_ids.contains(sid),
            "Unexpected subject id: {}",
            sid
        );
        assert!(
            txt.contains("HP:"),
            "No HP: terms found in output {:?}",
            p
        );
    }
}
