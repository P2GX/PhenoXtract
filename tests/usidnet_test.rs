...
use phenoxtract::{Pipeline, config::table_context::{Context, Identifier, SeriesContext, TableContext}, extract::{DataSource, ExcelDatasource, extraction_config::ExtractionConfig}, load::FileSystemLoader, ontology::{CachedOntologyFactory, OntologyRef, traits::HasPrefixId}, transform::{Collector, PhenopacketBuilder, TransformerModule, strategies::{AliasMapStrategy, MappingStrategy, MultiHPOColExpansionStrategy, OntologyNormaliserStrategy}, traits::Strategy}};
...
#[rstest]
fn test_cohort_import(
    usidnet_path: PathBuf,
    cohort_sheet_context: TableContext
) { 
    ...
    let mut data_sources = [   
        DataSource::Excel(ExcelDatasource::new(
            ...
            vec![
                ExtractionConfig::new("Cohort".to_string(), true, true),
            ],
        )),
    ]; 
    ...
    let assets_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join(PathBuf::from(file!()).parent().unwrap().join("usidnet_assets"));
