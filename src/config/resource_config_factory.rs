...
impl ResourceConfigFactory {
    pub fn build(&mut self, config: ResourceConfig) -> Result<Box<dyn BiDict>, FactoryError> {
        if config.prefix.to_uppercase() == KnownResourcePrefixes::LOINC.to_string() {
            Self::build_loinc_client(&config)
        } else {
            match self.ontology_factory.build_bidict(
                &ResourceRef::new(config.prefix.clone(), config.version.clone()),
                None,
            ) {
                ...
                Err(err) => {
                    let is_known = KnownResourcePrefixes::VARIANTS
                        .iter()
                        .any(|&known_id| known_id.eq_ignore_ascii_case(&config.prefix));
...
mod tests {
    ...
    fn test_build_loinc_success() {
        ...
        let config = ResourceConfig {
            prefix: KnownResourcePrefixes::LOINC.into(),
            version: None,
