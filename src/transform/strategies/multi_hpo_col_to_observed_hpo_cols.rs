use crate::config::table_context::Context::{MultiHpoId, SubjectId};
use crate::extract::contextualized_data_frame::ContextualizedDataFrame;
use crate::transform::error::TransformError;
use crate::transform::error::TransformError::StrategyError;
use crate::transform::strategies::utils::{convert_col_to_string_vec, get_hpo_ids_from_string};
...
#[allow(dead_code)]
pub struct MultiHPOColToObservedHpoColsStrategy;
impl Strategy for MultiHPOColToObservedHpoColsStrategy {
    fn is_valid(&self, table: &ContextualizedDataFrame) -> bool {
        table.check_contexts_have_data_type(MultiHpoId, DataType::String)
    }
    ...
    fn internal_transform(
        ...
        info!("Applying MultiHPOColToObservedHpoCols strategy to table: {table_name}");

        let subject_id_cols = table.get_cols_with_data_context(SubjectId);
        ...
        let stringified_subject_id_col = convert_col_to_string_vec(subject_id_col)?;

        let multi_hpo_cols: Vec<Column> = table
            .get_cols_with_data_context(MultiHpoId)
            .into_iter()
            .cloned()
            .collect();
        ...
        for multi_hpo_col in &multi_hpo_cols {
            let stringified_multi_hpo_col = convert_col_to_string_vec(multi_hpo_col)?;
            let patient_id_multi_hpo_pairs: Vec<(&String, &String)> = stringified_subject_id_col
                .iter()
            ...
            for (patient_id, multi_hpo) in patient_id_multi_hpo_pairs {
                ...
                let patient_hm_entry = patient_to_hpo
