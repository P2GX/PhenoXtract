...
impl PhenopacketBuilder {
    ...
    pub fn upsert_interpretation(
        ...
        if valid_linking_case_one {
            let _progress_status = "UNSOLVED";

            // etc.
        }

        if valid_linking_case_two {
            let _progress_status = "SOLVED";

            let _interpretation_status = if genes.len() == 1 {
                "CAUSATIVE"
            } else {
                "CONTRIBUTORY"
            };
        ...
        if valid_linking_case_three {
            ...
            let _interpretation_status = if variants.len() == 1 {
                "CAUSATIVE"
            } else {
                "CONTRIBUTORY"
            };
